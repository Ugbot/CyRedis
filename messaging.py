"""
CyRedis Messaging Components

Queues, streams, pubsub, and other messaging patterns.
High-performance Cython implementations with Python wrappers.
"""

# Try to import optimized Cython implementations first
try:
    from cy_redis.messaging import CyReliableQueue
    _OPTIMIZED_MESSAGING_AVAILABLE = True
    print("✓ Using optimized Cython messaging components")
except ImportError:
    _OPTIMIZED_MESSAGING_AVAILABLE = False
    print("Warning: Optimized Cython messaging not available, using Python fallback")

# Import the original implementations as fallback
...
class ReliableQueue:
    ...
    Uses optimized Cython implementation when available.
    """
    ...
    def __init__(self, redis_client, queue_name: str, visibility_timeout: int = 30,
        ...
        if _OPTIMIZED_MESSAGING_AVAILABLE:
