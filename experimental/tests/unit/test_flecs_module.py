...
class TestCyGameModuleAPI:
    ...
    def test_module_manager_import(self):
        ...
        try:
            from cyredis_experimental.game.module_manager import CyGameModule
        except ImportError:
...
class TestFlecsRestore:
    def test_restore_world_reloads_entities(self, redis_client, module_loaded):
        ...
        try:
            ...
            # Destroy the in-memory world. Every FLECS command rejects an
            # unknown world outright, so the world being gone shows up as an
            # error rather than a negative answer.
            redis_client.execute_command(["FLECS.FINI", world_id])
            with pytest.raises(Exception, match="world not found"):
