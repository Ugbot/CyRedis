...
Requires a running Redis instance on port 6380 with cy_game.so loaded:
    ...
    redis-server experimental/cyredis_experimental/game/module/test-redis.conf --daemonize yes
...
MODULE_SO_PATH = os.path.abspath("experimental/cyredis_experimental/game/module/cy_game.so")
LUA_DIR = os.path.abspath("experimental/cyredis_experimental/game/lua")
...
def module_redis():
    ...
    try:
        ...
        pytest.skip(
            ...
            "Start it with: redis-server experimental/cyredis_experimental/game/module/test-redis.conf --daemonize yes"
        )
    ...
    try:
        ...
        pytest.skip(
            ...
            "Build/load it (see experimental/cyredis_experimental/game/README.md)."
        )
    return r
...
class TestFlecsEntityLifecycle:
    ...
    def test_has_component_unknown_entity(self, module_redis, world):
        assert (
            module_redis.execute_command("FLECS.HAS", world, "ghost", "Position") == 0
        )

    def test_setcomp_position(self, module_redis, world):
        spawn(module_redis, world, "z1", "p1", "player", 0, 0)
        module_redis.execute_command(
            "FLECS.SETCOMP", world, "p1", "Position", "x", "99.0", "y", "88.0"
        )
        x, y = get_pos(module_redis, world, "p1")
...
class TestPathfinding:
    ...
    def test_straight_path_no_obstacles(self, module_redis, world):
        ...
        result = module_redis.execute_command(
            "CYPATH.FIND", grid, "0", "0", "4", "0", "100"
        )
        coords = list(zip(result[::2], result[1::2]))
        assert (4, 0) in [(int(x), int(y)) for x, y in coords] or any(
            int(x) == 4 for x, _ in coords
        )
...
class TestPhysics:
    ...
    def test_circle_finds_nearby_entity(self, module_redis, world):
        ...
        result = module_redis.execute_command(
            "CYPHYS.CIRCLE", spatial, "5.0", "5.0", "3.0", "10"
        )
        assert len(result) >= 2
        eids = result[::2]
        assert "p1" in eids
    ...
    def test_circle_excludes_distant_entity(self, module_redis, world):
        ...
        spatial = f"cy:spatial:{{{world}:z1}}"
        result = module_redis.execute_command(
            "CYPHYS.CIRCLE", spatial, "0.0", "0.0", "5.0", "10"
        )
    ...
    def test_circle_sorted_by_distance(self, module_redis, world):
        ...
        spatial = f"cy:spatial:{{{world}:z1}}"
        result = module_redis.execute_command(
            "CYPHYS.CIRCLE", spatial, "0.0", "0.0", "10.0", "10"
        )
        eids = result[::2]
...
class TestGOAP:
    ...
    def _setup_combat_actions(self, r, world):
        ...
        r.execute_command(
            "CYGOAP.SETSTATE", ws, "has_enemy", "0", "is_alive", "1", "has_ammo", "1"
        )
        r.execute_command(
            "CYGOAP.DEFACTION",
            actions,
            "attack",
            '{"has_enemy":1,"has_ammo":1}',
            '{"has_enemy":0}',
            "1",
        )
        r.execute_command(
    ...
    def test_plan_single_action(self, module_redis, world):
        ...
        plan = module_redis.execute_command(
            "CYGOAP.PLAN", ws, actions, "has_enemy", "1", "10"
        )
        assert plan == ["scout"]
    ...
    def test_plan_two_step(self, module_redis, world):
        ...
        module_redis.execute_command(
            "CYGOAP.SETSTATE", ws, "has_enemy", "1", "is_alive", "1", "has_ammo", "0"
        )
        module_redis.execute_command(
            "CYGOAP.DEFACTION",
            actions,
        ...
        plan = module_redis.execute_command(
            "CYGOAP.PLAN", ws, actions, "has_enemy", "0", "10"
        )
    ...
    def test_plan_unsatisfiable(self, module_redis, world):
        ...
        module_redis.execute_command(
            "CYGOAP.DEFACTION", actions, "noop", '{"alive":1}', '{"alive":1}', "1"
        )
        plan = module_redis.execute_command(
            "CYGOAP.PLAN", ws, actions, "has_magic", "1", "5"
        )
        assert plan == [] or plan is None or len(plan) == 0
...
class TestLuaGameplayFunctions:
    def test_flecs_tick_via_lua(self, module_redis, lua_loaded, world):
        ...
        result = module_redis.execute_command(
            "FCALL", "flecs_tick", "1", world, "z1", "1000", "256"
        )
        d = {}
        for i in range(0, len(result) - 1, 2):
            d[result[i]] = result[i + 1]
    ...
    def test_flecs_snapshot_via_lua(self, module_redis, lua_loaded, world):
        spawn(
            module_redis, world, "z1", "p1", "player", 3.0, 4.0, vx=1.0, vy=2.0, hp=80
        )
        result = module_redis.execute_command(
            "FCALL", "flecs_snapshot", "1", world, "p1"
        )
        d = {}
        it = iter(result)
        for k in it:
    ...
    def test_flecs_apply_damage_reduces_hp(self, module_redis, lua_loaded, world):
        spawn(module_redis, world, "z1", "p1", "player", 0, 0, hp=100)
        result = module_redis.execute_command(
            "FCALL", "flecs_apply_damage", "1", world, "p1", "30"
        )
        assert result[0] == 1
    ...
    def test_flecs_apply_damage_kills(self, module_redis, lua_loaded, world):
        ...
        result = module_redis.execute_command(
            "FCALL", "flecs_apply_damage", "1", world, "p1", "50"
        )
        assert result[0] == 1
    ...
    def test_ai_goap_plan_via_lua(self, module_redis, lua_loaded, world):
        ...
        actions = f"cy:actions:{{{world}}}"
        module_redis.execute_command(
            "CYGOAP.SETSTATE", ws, "alive", "1", "target_found", "0"
        )
        module_redis.execute_command(
            "CYGOAP.DEFACTION",
            actions,
...
class TestPythonDriverIntegration:
    ...
    def game_engine(self, module_redis):
        ...
        try:
            from cyredis_experimental.game.game_engine import GameEngine
        except ImportError:
            pytest.skip("cyredis_experimental.game not built")
