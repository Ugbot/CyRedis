...
from cyredis_experimental.auth.session_manager import SessionManager
from cyredis_experimental.auth.token_manager import TokenManager
from cyredis_experimental.auth.two_factor_auth import TwoFactorAuth
from cy_redis.data.shared_dict import CySharedDict
from cyredis_experimental.web.shared_state_manager import SharedStateManager
...
from cyredis_experimental.workers.worker_queue import WorkerQueue
...
class WebAppSupport:
    ...
    def get_shared_dict(self, name: str) -> CySharedDict:
        """Get a concurrent shared dictionary by name"""
        return CySharedDict(self.redis_client, f"shared_dict:{name}")

    def create_shared_dict(self, name: str, initial_data: Dict[str, Any] = None) -> CySharedDict:
        """Create a new concurrent shared dictionary"""
        shared_dict = CySharedDict(self.redis_client, f"shared_dict:{name}")
        if initial_data:
